...
def render_session_context_markdown(pack: SessionContextPack) -> str:
    ...
    if pack.available_capabilities:
        ...
        sections.append("")
        sections.append(
            "Use the `/subagents spawn` command to create a sub-agent:"
        )
        sections.append("")
        sections.append("```")
        sections.append(f'/subagents spawn coding "Review the authentication middleware for security vulnerabilities" --model {coding_model}')
        sections.append("```")
        sections.append("")
        sections.append("**Syntax:** `/subagents spawn <agentId> <task> [--model <model>] [--thinking <level>]`")
        sections.append("")
        sections.append("**Key points:**")
        sections.append(f"- Use `--model {coding_model}` for coding tasks")
        sections.append("- The `<agentId>` is a label for the sub-agent (e.g. `coder`, `reviewer`, `debugger`)")
        ...
        sections.append("- CLARKE automatically injects matched skill methodologies and policies into the sub-agent")
